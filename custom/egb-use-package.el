...
(defmacro egb-use-package (el-get-form &rest use-package-forms)
  ...
  (let* ((el-get-features-present2 (and use-package-forms (eq :features (car use-package-forms))))
         (pkgname-present2 (if el-get-features-present2
                               (eq :pkgname (car (cddr use-package-forms)))
                             (and use-package-forms (eq :pkgname (car use-package-forms)))))
         (el-get-features (if el-get-features-present2
                              (list (car use-package-forms) (cadr use-package-forms))))
         (pkgname (if pkgname-present2
                      (if el-get-features-present2
                          (nth 3 use-package-forms)
                        (nth 1 use-package-forms))
                    el-get-form))
         (use-package-forms (if el-get-features-present2
                                (if pkgname-present2
